...
import URI, { decodeQuery } from "urijs";
...
const decodeAndParseStartHash = url => {
  let config = new URL(url).search.replace("?start=", "");
  let decoded = decodeQuery(config);
...
describe("BuildShareLink", function() {
  ...
  describe("should generate a url that opens to the catalog", function() {
    xit("when the explorer window is open without a previewed catalog item", function(done) {
      catalog
    ...
    describe("opening to the user added tab", function() {
      ...
      //     .otherwise(done.fail);
      // });
      xit("viewing a previewed item", function(done) {
        catalog
          .updateFromJson(catalogUpdateJson)
          .then(function() {
