...
describe("CsvCatalogItem with no geo using default bundled regionMapping", function() {
  ...
  xit("does not automatically enable a column if a table style is loaded in without a dataVariable", function(done) {
    csvItem.url = "test/csv_nongeo/xy.csv";
    csvItem
      .updateFromJson({
        tableStyle: {
...
describe("CsvCatalogItem & chart sharing", function() {
  ...
  describe("disableIncompatibleTableColumn interaction", function() {
    describe("should not disable other charted columns if there are no active columns in use", function() {
      xit("activates time series columns when loading the time series last", function(done) {
      ...
      xit("activates scalar columns when loading the time series first", function(done) {
        const xyCsv = new CsvCatalogItem(terria);
        const timeSeriesCsv = new CsvCatalogItem(terria);
        timeSeriesCsv
          .updateFromJson({
  ...
  describe("serialization around tableStyle & tableStructures for non-geo time series csvs", function() {
    ...
    xit("generates columns on a table style on serialization for chartable items, when a CsvCatalogItem is created without them", function(done) {
      columns = {
        "0": {
          active: false
        },
    ...
    xit("initialises and shares the correct 'no variables selected' state", function(done) {
      columns = {
        "0": {
          active: false
        },
    ...
    xit("initialises the correct 'second variable is selected' state & shares newly toggled state", function(done) {
      columns = {
        "0": {
          active: false
        },
