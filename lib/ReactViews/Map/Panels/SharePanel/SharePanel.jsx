...
import PrintUsingTemplateView from "./PrintUsingTemplateView";
import VisualizationType from "../../../../Models/VisualizationType";
import Collapsible from "../../../Collapsible/Collapsable";
...
const SharePanel = createReactClass({
  ...
  customPrint() {
    this.createPrintUsingTemplateView(true);
  },

  createPrintUsingTemplateView(hidden) {
    this.setState({
  ...
  renderContentWithPrintAndEmbed() {
    ...
    const canShowDownloadSection =
      terria.viewType == VisualizationType.TABLE && terria.nowViewingTable;
    const bulkDownloadLinks =
      (canShowDownloadSection &&
        terria.nowViewingTable.tabularViewSettings?.bulkDownloadUrls) ||
      [];
    const canShowBulkDownloadSection =
      Array.isArray(bulkDownloadLinks) && bulkDownloadLinks.length;
    ...
    return (
      <div>
        ...
        <If condition={terria.viewType != VisualizationType.TABLE}>
          <div className={DropdownStyles.section}>
            ...
            <div>
              ...
              <button
                className={Styles.printButton}
                onClick={this.customPrint}
                disabled={this.state.creatingPrintView}
                title={t("share.customPrintFinePrint")}
              >
                {t("share.customPrint")}
              </button>
        ...
        <If condition={canShowDownloadSection}>
          {
            <fieldset className={Styles.downloadContainer}>
              <legend>
                {" "}
              ...
              <div>
                ...
                {!this.state.preparingExcel && (
                  <div>
                    <button
                      aria-label={t("share.downloadExcel")}
                      title={t("share.downloadExcel")}
                      onClick={() => {
                        this.setState({ preparingExcel: true });
                        terria._initiateTableDownload({ excel: true });
                        setTimeout(
                          () => this.setState({ preparingExcel: false }),
                          1000
                        );
                      }}
                      className={classNames(
                        Styles.download,
                        Styles.downloadExcel
                      )}
                    >
                    ...
                    <button
                      aria-label={t("share.downloadCsv")}
                      title={t("share.downloadCsv")}
                      onClick={() =>
                        this.props.terria._initiateTableDownload({ csv: true })
                      }
                      className={classNames(
                        Styles.download,
                        Styles.downloadCsv
                ...
                <If condition={canShowBulkDownloadSection}>
                  <Collapsible name={t("share.bulkDownload")}>
                    <div className={Styles.bulkDownloadSection}>
                      <ul>
                        {bulkDownloadLinks.map((a, i) => {
                          return (
                            <li key={i}>
                              {" "}
                              <a
  ...
  renderTemplateSelector() {
    if (!this.state.printOptions.showTemplateSelector) return null;

    return (
      this.state.printOptions.shareUrl && (
        <PrintUsingTemplateView
          options={{ shareUrl: this.state.printOptions.shareUrl }}
          terria={this.props.terria}
          readyCallback={() => {
            this.setState({
              creatingPrintView: false,
  ...
  render() {
    ...
    if (terria.viewType == VisualizationType.TABLE) {
      btnText = t("share.btnTableShareText");
      btnTitle = t("share.btnTableShareTitle");
    } else {
      btnText = catalogShare
        ? t("share.btnCatalogShareText")
        : storyShare
        ? t("share.btnStoryShareText")
        : t("share.btnMapShareText");

      btnTitle = catalogShare
