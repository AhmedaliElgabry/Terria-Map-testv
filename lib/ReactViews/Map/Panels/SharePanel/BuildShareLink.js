...
import { TableTypes } from "../../../TableView/TableTypes";
...
export function getShareData(
  ...
  addLocationMask(terria, initSources);
  addTable(terria, initSources);
      ...
      itemFilter: combineFilters([
        function(item) {
          if (CatalogMember.itemFilters.noLocalData(item)) {
            ...
            item.polygonForAreaTimeSeries ||
            (item.customProperties &&
              item.customProperties.areaAnalysisParameter)
          ) {
            return true;
          }
...
function addViewSettings(terria, viewState, initSources) {
  ...
  const terriaSettings = {
    ...
    activeProjection: terria.activeProjection,
    baseMapName: terria.baseMap.name,
    viewerMode: viewerMode,
    currentTime: time,
    viewType: terria.viewType
  };
...
/**
 * Add information about the focus mask layer.
 * @private
 */
function addLocationMask(terria, initSources) {
  const maskContext = terria.currentViewer._maskLayerContext;
  if (!maskContext || !maskContext.layerName) return;

  const mask = {
    sourceLayer: maskContext.layerName,
    featureId: maskContext.featureId,
    searchKey: maskContext.searchKey,
    searchValue: maskContext.searchValue,
    rectangle: maskContext.rectangle,
    tilebox: maskContext.tilebox,
    hash: maskContext.hash,
    providerCoords: maskContext.providerCoords,
    pickCoords: maskContext.pickCoords
  };
...
function addTable(terria, initSources) {
  if (terria.nowViewingTable && terria.viewType == VisualizationType.TABLE) {
    const uniqueId = terria.nowViewingTable.uniqueId;

    const table = {
      currentTable: uniqueId,
      tableType: terria.tableType,
      tableTop: terria.tableTop || 0,
      tableSortColumns: {
        [uniqueId]: terria.tableSortColumns?.[uniqueId] || []
      },
      tableFilters: { [uniqueId]: terria.tableFilters?.[uniqueId] || {} }
    };

    if (terria.tableType == TableTypes.pivot && terria.currentPivotViews) {
      if (terria.currentPivotViews[uniqueId])
        table.currentPivotView = {
          [uniqueId]: terria.currentPivotViews[uniqueId]
        };
    }
