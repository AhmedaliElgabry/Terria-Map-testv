...
import AnalysisButtonStyle from "./analysis-button.scss";
import AnalysisButton from "./AnalysisButton";
import ExportButton from "./ExportButton";

import SepalButton from "./SepalButton";
import Cartographic from "terriajs-cesium/Source/Core/Cartographic";
import Rectangle from "terriajs-cesium/Source/Core/Rectangle";
import FeatureInfoLink from "./FeatureInfoLink";
...
export const FeatureInfoSection = createReactClass({
  ...
  getDimensionData(item) {
    if (!item.dimensions) return "";

    const dimensions = Object.entries(item.dimensions);
    const dimensionLabels = Object.entries(item.dimensionLabels || []);

    const rows = dimensions.map(dim => {
      const caption = dimensionLabels.find(a => a[0] === dim[0]);
      const label =
        (caption && caption.length === 2 ? caption[1] : dim[1]) || dim[1];
    ...
    return <table className={Styles.dimensionTable}>{rows}</table>;
  },
  getTemplateData() {
    ...
    if (defined(propertyData)) {
      ...
      propertyData.fao = {
        math: faoMath,
        formatMonths: faoMonthFormatter,
        toLower: faoToLower,
        isNull: faoIsNull
      };
  ...
  hasTemplate() {
    ...
    // Need to find a better way to calculate if there is a template but Item has no data. This could be true for region mapping items
    // below is a good enough solution saying that if the item is smart-csv or csv. Check if it has some numerical properties.
    if (catalogItem.type === "smart-csv" || catalogItem.type === "csv") {
      const templateData = this.getPropertyValues();
      const templateDataExists =
        defined(templateData) &&
        templateData.hasOwnProperty("_terria_numericalProperties") &&
        Object.keys(templateData._terria_numericalProperties).length !== 0;
  ...
  onFocusClick() {
    const { terria, catalogItem } = this.props;
    const { pickPosition, providerCoords } = terria.pickedFeatures;

    if (catalogItem.type == "geojson") {
      return;
    }

    const bboxRadians = 0.35;
    ...
    const latLonPosition = Ellipsoid.WGS84.cartesianToCartographic(
      selectedFeature.position.getValue(),
      cartographicScratch
    );

    terria.currentViewer._createMaskAroundFeature(selectedFeature, {
      providerCoords,
      pickCoords: {
    ...
    terria.currentViewer.zoomTo(rectangle, 2);

    // Rerender is necessary
    // But we do not maintain currentViewer's state in this component
    this.forceUpdate();
  },
  componentDidMount() {
    if (!this.rootRef.current) return;
    /** @type {HTMLElement} */
    const rootElt = this.rootRef.current;
    /** @todo Change this setTimeout to a mustache render complete call-back */
    setTimeout(() => {
      const links = [...rootElt.querySelectorAll(".geojson-download")];
      links.forEach(lnk => {
        lnk.disabled = false;
        lnk.addEventListener("click", e => {
          const data = lnk.nextSibling.innerText;
          const link = document.createElement("a");
          link.setAttribute(
            "href",
            "data:application/json;charset=utf-8," + encodeURIComponent(data)
          );
          link.setAttribute("download", lnk.getAttribute("data-file"));
  ...
  render() {
    const {
      t,
      terria: {
        configParameters: { useDrawAreaForAnalysis }
      },
      terria,
      catalogItem
    } = this.props;

    const catalogItemName = (catalogItem && catalogItem.name) || "";
    ...
    const customProperties = catalogItem.customProperties || {};

    const showDimensionsOnPanel =
      customProperties.showDimensionsOnPanel ||
      customProperties.dimensionLabelsConfig?.showDimensionsOnPanel ||
      false;

    // eslint-disable-next-line eqeqeq
    const hideAllUi = terria.userProperties.hideAllUi == "1";

    const isGeojson = catalogItem.type === "geojson";
    const isMvt = catalogItem.type === "mvt";
    const selectedFeature = terria.selectedFeature;
    const isMaskOn = isMvt && terria.currentViewer._isMaskOn(selectedFeature);

    return (
      <li className={classNames(Styles.section)} ref={this.rootRef}>
        <If condition={this.props.printView}>
          <h2>{fullName}</h2>
        </If>

        <If condition={!customProperties.hideInFeatureInfo}>
          <button
            type="button"
        ...
        <If
          condition={this.props.isOpen && !customProperties.hideInFeatureInfo}
        >
          <section className={Styles.content}>
            <If condition={showDimensionsOnPanel}>
              {this.getDimensionData(this.props.catalogItem)}
            </If>
            ...
            <If condition={customProperties.allowAnalysis && !hideAllUi}>
              <AnalysisButton
                viewState={this.props.viewState}
                terria={this.props.terria}
                catalogItem={this.props.catalogItem}
            ...
            <If
              condition={
                (isMvt || isGeojson) && catalogItem.allowMasking && !hideAllUi
              }
            >
              <button
                type="button"
                title={
                  isMaskOn
                    ? t("featureInfo.restoreOriginalView")
                    : t("featureInfo.focusSelectedFeature")
                }
                className={AnalysisButtonStyle.focusButton}
                onClick={this.onFocusClick}
              >
                {isMaskOn ? (
                  <>
                    <Icon glyph={Icon.GLYPHS.close} />{" "}
                    {t("featureInfo.original")}
                  </>
                ) : (
                  <>
                    <Icon glyph={Icon.GLYPHS.eye} /> {t("featureInfo.focus")}
                  </>
                )}
              </button>
            </If>
            <If
              condition={
                (customProperties.crtb || isMvt || isGeojson) &&
                useDrawAreaForAnalysis &&
                !hideAllUi
              }
            >
              <button
                type="button"
                className={AnalysisButtonStyle.analysisButton}
                onClick={() =>
                  this.props.terria.setCrtbModalVisibility({
                    visibility: true,
                    catalogItem: this.props.catalogItem,
                    analysisTypes:
                      customProperties?.allowAnalysis?.analysisTypes,
                    autoSelectDrawnArea:
                      this.props.catalogItem.type === "geojson" &&
                      useDrawAreaForAnalysis
              ...
              >
                <Icon glyph={Icon.GLYPHS.lineChart} /> {t("featureInfo.crtb")}
              </button>
            </If>

            <If
              condition={
                !customProperties.hideRawDataButton &&
            ...
            <div>
              <For
                each="CustomComponent"
                index="i"
                of={FeatureInfoSection._customComponents}
              >
                <CustomComponent
                  key={i}
 ...
 */
export function getPropertyValuesForFeature(feature, currentTime, formats) {
...
function faoMath() {
  return function(text, render) {
    ...
    try {
      ...
      const expression = parser.parse(renderedText);

      const value = expression.evaluate();

      return value;
    } catch (error) {
      console.warn("Math ops failed", error);
      return "";
    }
  };
}
...
// converts "2020-060708" to "June, July, August 2020"
function faoMonthFormatter() {
  return function(text, render) {
    const renderedText = render(text);

    try {
      const re = /(\d{4})-(\d{12}|\d{10}|\d{8}|\d{6}|\d{4}|\d{2})/;
      const matches = re.exec(renderedText);

      if (!matches || matches.length < 3) {
        return renderedText;
      }

      const year = matches[1]; // 2020
      const monthString = matches[2]; // 080910

      if (monthString.length % 2 !== 0) {
        return renderedText;
      }

      const monthNumbers = []; // [08, 09, 10]

      for (let i = 0; i < monthString.length; i += 2) {
        monthNumbers.push(monthString.substr(i, 2));
      }

      const monthText = monthNumbers
        .map(month => Number(month))
        .sort((a, b) => (a <= b ? -1 : 1))
        .map(month => (month >= 0 && month <= 12 ? months[month - 1] : month))
        .join(", "); // August, September, October

      const convertedText = renderedText.replace(re, `${monthText} ${year} `);

      return convertedText;
    } catch (error) {
      console.warn("Formatting date failed", error);
      return render(text);
    }
  };
}

// {{fao.isNull}} {{Value}}, Default {{#fao.isNull}}
// if {{Value}} is not null, then it is rendered
// if {{Value}} is evaluated to null, then "Default" is rendered
function faoIsNull() {
  const isEmpty = str => !str || /^\s*$/.test(str);
  return function(text, render) {
    const renderedText = render(text);

    if (!text.includes(",")) return renderedText;

    const templateParts = text.split(","); // ["{{Value}}", "Default"]
    const renderedParts = renderedText
      .split(",")
      .filter(part => !isEmpty(part)); // ["Val", "Default"] OR ["Default"] if {{Value}} is null
    ...
    if (renderedParts.length === 1) {
      // it only has the default value
      return renderedParts[0]; // return the default
    }

    renderedParts.pop(); // remove the default which should be the last value in the array

    return renderedParts.join(" "); // using array.Join(' ') instead of renderedParts[0] because potentially {{Value}} could resolve to string that has a comma in it
  };
}
...
FeatureInfoSection._customComponents = [FeatureInfoLink];
