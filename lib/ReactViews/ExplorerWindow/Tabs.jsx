...
const Tabs = createReactClass({
    ...
    if (this.props.terria.configParameters.tabbedCatalog) {
      return [].concat(
        this.props.terria.catalog.group.items
          ...
          .map((member, i) => ({
            name: he.decode(member.nameInCatalog),
            title: `data-catalog-${he.decode(member.name || "")}`,
  ...
  render() {
    ...
    // Only activate tab if there is no previewed item, this is to prevent 'About this data' functionality from breaking
    if (currentTab && !this.props.viewState.previewedItem) {
      this.activateTab(currentTab.category, currentTab.idInCategory);
    }
