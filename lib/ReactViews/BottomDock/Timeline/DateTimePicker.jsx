...
const DateTimePicker = createReactClass({
  ...
  renderYearGrid(datesObject) {
    ...
    if (datesObject.dates && datesObject.dates.length > 12) {
      ...
      return (
        <div className={Styles.grid}>
          <div className={Styles.gridHeading}>{t("timePicker.selectYear")}</div>
  ...
  renderList(items) {
    ...
    if (defined(items)) {
      return (
        <div className={Styles.grid}>
          <div className={Styles.gridHeading}>{t("timePicker.selectTime")}</div>
          <div className={Styles.gridBody}>
            {items.map(item => (
              <button
  ...
  renderHourView(datesObject) {
    ...
    if (timeOptions.length > 24) {
      return (
        <div className={Styles.grid}>
          <div className={Styles.gridHeading}>
            {`${t("timePicker.selectAnHour")} ${this.state.day} ${
