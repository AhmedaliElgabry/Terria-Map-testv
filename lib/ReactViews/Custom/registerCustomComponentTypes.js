...
const registerCustomComponentTypes = function(terria) {
  ...
  function processChartNode(context, node, children) {
    ...
    // adjust the naming with karl
    const chartItemName = [context.catalogItem.name, yColumns.toString()];

    if (
      context.catalogItem.hasOwnProperty("Selection") &&
      context.catalogItem.hasOwnProperty("SmartCsvDimensions")
    ) {
      const { Dimensions } = context.catalogItem.SmartCsvDimensions;
      const labels = Dimensions.map(dim => {
        if (context.catalogItem.Selection.hasOwnProperty(dim.Id)) {
          return (
            dim.Values.find(
              x => x.value === context.catalogItem.Selection[dim.Id]
            )?.label || ""
          );
        }
      });

      chartItemName.push(...labels);
    }

    const url = defined(sources) ? sources[0] : undefined;
    let csvStr = getSourceData(node, children);
    const csvArray = removeInactiveColumns(context, xColumn, yColumns, csvStr);
    const shouldChangeColumnName = yColumns.length === 1;
    const properties =
      context.feature.currentProperties ||
      propertyGetTimeValues(
        context.feature.properties,
        context.catalogItem.currentTime
      );
    const propertiesKeySet = Object.keys(properties);
    // change the header name when the ycolumn is only 1. Ask karl how to best handle more than 1 column.
    if (shouldChangeColumnName) {
      let name = yColumns[0];
      if (context.feature._name) {
        name = context.feature._name;
      } else if (
        propertiesKeySet.length !== 0 &&
        context.catalogItem.customProperties?.feature?.featureName &&
        ...
        const nameKey =
          context.catalogItem.customProperties.feature.featureName;
        name = properties[nameKey];
      }
      const yColumnIndex = csvArray[0].indexOf(yColumns[0]);

      yColumns[0] = csvArray[0][yColumnIndex] = name;
    }

    csvStr = arrayToCSV(csvArray);

    const tableStructure = tableStructureFromStringData(csvStr);
    ...
    if (node.attribs["hide-buttons"] !== "true") {
      chartElements.push(
        React.createElement(ChartExpandAndDownloadButtons, {
          ...
          title: title,
          chartItemName: chartItemName.join(" - "),
  ...
  /**
   * Removes all the unnecessary columns from the csvString and if we have only one yColumn replace it with context.title
   * @private
   */
  function removeInactiveColumns(context, xColumn, yColumns, csvStr) {
    const array = csv.toArrays(csvStr);
    const allowedHeaders = yColumns.concat(xColumn);
    const headers = array[0];
    const indicesToPreserve = allowedHeaders.map(allowedHeader => {
      return headers.indexOf(allowedHeader);
    });
    return array.map(row => {
      return row.filter(function(value, index) {
        return indicesToPreserve.indexOf(index) !== -1;
      });
    });
  }
  ...
  function tableStructureFromStringData(stringData) {
    ...
    // load the new or existing tableStructure here
