...
const ChartExpandAndDownloadButtons = createReactClass({
  propTypes: {
    ...
    chartItemName: PropTypes.string,
...
function expand(props, sourceIndex) {
  ...
  function makeNewCatalogItem() {
    ...
    let newCatalogItem = props.terria.nowViewing.items.find(
      item => item.type === "csv" && item.name === props.chartItemName
    );
    if (!defined(newCatalogItem)) {
      newCatalogItem = new CsvCatalogItem(terria, url, {
      ...
      const name = props.chartItemName;
      newCatalogItem.name =
        name || props.title || (props.feature && props.feature.name) || "Chart";
    }

    // We know that we have a specific catalog item.
    if (
      newCatalogItem &&
      newCatalogItem.data instanceof TableStructure &&
      newCatalogItem.data.hasOwnProperty("csvStr")
    ) {
      // take the new tableStructure and use its csvStr property and apply a merge with the existing str policy
      // generate a new TableStrucure and set the data.
      const { csvStr } = newCatalogItem.data;
      const normalizedCsvStr = tableStructure.mergeCsvStrings(
        csvStr,
        tableStructure.csvStr,
        {
          xAxis: newCatalogItem._tableStyle.xAxis
        }
      );
      tableStructure = TableStructure.fromCsv(
        normalizedCsvStr.replace(/\\n/g, "\n"),
        tableStructure
      );
    ...
    if (props.catalogItem.customProperties.highchartsOption)
      newCatalogItem.customProperties.highchartsOption = highchartDefaultOption;
    // const items = [newCatalogItem];
    // if (defined(newCatalogItem.creatorCatalogItem)) {
    //   debugger;
    //   const existingAreas = newCatalogItem.creatorCatalogItem.items.filter(
    //     item =>
    //       item instanceof GeoJsonCatalogItem &&
    //       item.uniqueId !== props.yColumns.join(" - ")
    //   );
    //   items.push(...existingAreas);
    // }
    ...
    //   newGeoJsonItem.name = props.yColumns.join(" - ");
    //   newGeoJsonItem.isUserSupplied = true;
    //   newGeoJsonItem.isComposite = true; // remove this when karl confirms
    ...
    // }
    // newCatalogItem.creatorCatalogItem = undefined;
    // items.unshift(newCatalogItem);
    // const compositeItem = new CompositeCatalogItem(terria, items);
    // compositeItem.customProperties = compositeItem.customProperties || {};
    // compositeItem.customProperties.highchartsOption = highchartDefaultOption;
    ...
    // debugger; //isHighcharts need to change the default parameters for this method
    const highchartsOptions = newCatalogItem.getHighchartOptions(
      { chartConfig: {}, isGrouping: true, name: newCatalogItem.name },
      {},
      props.yColumns.join(" - "),
      tableStructure,
      newCatalogItem.name
    );
    newCatalogItem.customProperties.highchartsOption = highchartsOptions;
    ...
    terria.catalog.addChartableItem(newCatalogItem);
  ...
  newCatalogItem.load().then(() => {
    ...
    try {
      ...
      let csvItem = newCatalogItem;
      // for (let i = 0; i < newCatalogItem.items.length; i++) {
      //   if (
      //     newCatalogItem.items[i].type === "csv" ||
      //     newCatalogItem.items[i].type === "smart-csv"
      //   )
