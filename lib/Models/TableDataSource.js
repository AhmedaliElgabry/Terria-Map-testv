...
const { filterValue } = require("../ReactViews/TableView/Grid/SortFilter");
...
var TableDataSource = function(
  ...
  knockout
    ...
    .subscribe(_changedActiveItems.bind(null, this), this);
};
...
function filterEntity(entity, filters) {
  ...
  for (let filter of filterPairs) {
    const entityValue = entity[filter[0]]?.toString();
    const criteria = filter[1];

    if (!defined(entityValue)) {
      continue;
    }

    pass = filterValue(criteria, entityValue);
...
function setOneFeaturePerRow(dataSource, tableStructure, specialColumns) {
  ...
  for (i = 0; i < dataSource._rowObjects.length; i++) {
    ...
    if (dataSource.filterContext.flattenTime) {
      const shouldShow = filterEntity(
        dataSource._rowObjects[i]?.number || rowObject,
        dataSource.filterContext.filters
      );
...
function setOneFeaturePerId(dataSource, tableStructure, specialColumns) {
  ...
  for (var key in rowNumbersMap) {
    if (rowNumbersMap.hasOwnProperty(key)) {
      ...
      // Only filter over flattened time dataset
      if (dataSource.filterContext.flattenTime) {
        const shouldShow = filterEntity(
          dataSource._rowObjects[rowNumbersMap[key][0]].number || firstRow,
          dataSource.filterContext.filters
        );
...
function updateEntitiesAndExtent(dataSource) {
  ...
  var specialColumns = {
    ...
    time: dataSource.filterContext.filters
      ? undefined
      : tableStructure.activeTimeColumn,
