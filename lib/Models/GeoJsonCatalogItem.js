...
GeoJsonCatalogItem.prototype.saveGeometry = function() {
  const data = {
    type: "geojson",
    name: this.name,
    data: this.data,
  ...
  return axios.post(this.terria.configParameters.saveAreaUrl, data);
};
  ...
  require.ensure(
    ...
    function() {
      ...
      promiseFunctionToExplicitDeferred(codeSplitDeferred, function() {
        ...
        // discuss with karl about the possibility of extending this to all geojson data sources
        // if (
        //   that.isComposite &&
        //   that.terria.dataSources._dataSources.some(
        //     element => element.name === that.name
        //   )
        // ) {
        //   const dataSourcesToRemove = that.terria.dataSources._dataSources.filter(
        //     element => element.name === that.name
        //   );
        //   reAdd = dataSourcesToRemove
        //     .map(dataSource => that.terria.dataSources.remove(dataSource, true))
        //     .every(v => v === true);
        // }
...
function loadGeoJson(geoJsonItem) {
  ...
  return dataSource.load(geoJsonItem._readyData, options).then(function() {
    ...
    // Complex geojsons fail with the default arc type which is RHUMB
    // Setting arcType to GEODESIC.
    // More info on the issue can be found here
    // https://github.com/CesiumGS/cesium/issues/8042
    for (var i = 0; i < entities.length; i++) {
      var entity = entities[i];
      if (defined(entity.polygon)) {
        entity.polygon.arcType._value = 1; // Cesium.ArcType.GEODESIC;
      }
    }
...
function updateOpacity(item) {
  if (
    !item.dataSource ||
    !item.dataSource.entities ||
    !item.dataSource.entities.values
  )
    return;
