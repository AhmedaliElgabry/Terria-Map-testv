...
var ImageryLayerCatalogItem = function(terria) {
  ...
  /**
   * Decides if a progress bar is shown on the workbench item
   * @type {Boolean}
   */
  this.showLoadingProgressBar = false;
  /**
...
ImageryLayerCatalogItem.enableLayer = function(
            ...
            if (e.statusCode >= 400 && e.statusCode < 500) {
              if (
                ...
                (catalogItem.treat403AsError === false &&
                  e.statusCode === 403) ||
                (e.statusCode === 403 && catalogItem.type == "csv")
              ) {
                tellMapToSilentlyGiveUp();
              } else {
  ...
  if (layer) {
    const _tileLoadTracker = {
      catalogItem: catalogItem,
      onChange: function(loaded, total) {
        this.catalogItem.totalTiles = total;
        this.catalogItem.loadedTiles = loaded;
      }
    };

    layer._tileLoadTracker = _tileLoadTracker;
  }
