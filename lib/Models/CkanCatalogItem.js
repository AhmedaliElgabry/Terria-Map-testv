...
function CkanCatalogItem(terria) {
  ...
  /**
   * Gets or sets a url that will be used to set @itemProperties
   * @type {URL}
   */
 ...
 * @param {Object} [options.itemPropertiesUrl] Additional properties URL to apply to the item once created.
...
CkanCatalogItem.createCatalogItemFromResource = function(options) {
  ...
  } else if (options.itemPropertiesUrl) {
    var _cachedValue = localStorage.getItem(options.itemPropertiesUrl);

    if (_cachedValue) {
      newItem.updateFromJson(JSON.parse(_cachedValue));
      loadJson(options.itemPropertiesUrl).then(function(data) {
        localStorage.setItem(options.itemPropertiesUrl, JSON.stringify(data));
      });
    } else {
      loadJson(options.itemPropertiesUrl).then(function(data) {
        localStorage.setItem(options.itemPropertiesUrl, JSON.stringify(data));
        newItem.updateFromJson(data);
      });
    }
  }
...
CkanCatalogItem.prototype._load = function() {
  ...
  return datasetIdPromise.then(function(datasetId) {
    ...
    return loadJson(datasetUrl).then(function(json) {
      ...
      var extras = json.result.extras;

      var itemPropertiesFromSource = {
        hasItemProperties: null,
        isUrl: null,
        value: null
      };
      ...
      if (extras) {
        extras.find(extr => {
          var hasTerriaIP =
            (extr.key || "")
              .toLowerCase()
              .indexOf("terriajs:itemproperties") !== -1;
          itemPropertiesFromSource.hasItemProperties = hasTerriaIP;

          if (hasTerriaIP) {
            var urlRegex = /(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g;
            itemPropertiesFromSource.isUrl = urlRegex.test(extr.value);
            itemPropertiesFromSource.value = extr.value;
          }
      ...
      for (var i = 0; i < resourcesToConsider.length; ++i) {
        var catalogItem = CkanCatalogItem.createCatalogItemFromResource({
          ...
          itemProperties:
            itemPropertiesFromSource.hasItemProperties &&
            !itemPropertiesFromSource.isUrl
              ? JSON.parse(itemPropertiesFromSource.value)
              : that.itemProperties,
          itemPropertiesUrl:
            itemPropertiesFromSource.hasItemProperties &&
            itemPropertiesFromSource.isUrl
              ? itemPropertiesFromSource.value
              : null
        });
