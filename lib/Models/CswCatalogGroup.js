...
var CswCatalogGroup = function(terria) {
  ...
  /**
   * True to add child items of wms groups as direct decendents of csw, thereby creating a "flat" structure; otherwise, false.
   * @type {Boolean}
   * @default false
   */
  ...
  function loadNextPage() {
    ...
    return loadWithXhr({
      ...
      for (var i = 0; i < records.length; ++i) {
        ...
        for (let j = 0; j < acceptableUrls.length; ++j) {
          ...
          if (defined(group)) {
            ...
            if (defined(catalogItem)) {
              if (
                that.forceFlat &&
                catalogItem instanceof WebMapServiceCatalogGroup
              ) {
                group.flatStructureLoading = true;

                catalogItem.toggleOpen();

                group.isLoading = catalogItem.isLoading;

                knockout
                  .getObservable(catalogItem, "isLoading")
                  // disabled because not modifying variables outside of the loop.
                  // eslint-disable-next-line no-loop-func
                  .subscribe(isLoading => {
                    if (isLoading) return;
                    group.flatStructureLoading = false;

                    for (const item of catalogItem.items) {
                      item.description =
                        (catalogItem.description || "") +
                        "\n\n" +
                        item.description;

                      if (catalogItem.info && catalogItem.info.length > 0) {
                        item.info = item.info || [];

                        for (const info of catalogItem.info) {
                          item.info.push(info);
                        }
                      }
...
function createItemForUri(catalogGroup, record, uri, downloadUrls, legendUrl) {
  ...
  if (defined(catalogItem)) {
    ...
    if (catalogItem instanceof WebProcessingServiceCatalogGroup) {
      ...
      // try to recover from malformed description metadata coming from geonetwork
      ...
      catalogItem.info.push({
        ...
        content: defined(catalogGroup.baseLink)
          ? (catalogGroup.baseLink + "/" + record.identifier).replace(
              /([^:]\/)\/+/g,
              "$1"
            )
          : catalogGroup.url +
      ...
      if (typeof catalogGroup.itemProperties === "object") {
        if (catalogItem instanceof WebMapServiceCatalogGroup) {
          catalogItem.updateFromJson({
            itemProperties: catalogGroup.itemProperties
          });
        } else {
