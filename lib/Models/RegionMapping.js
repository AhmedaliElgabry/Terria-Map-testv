...
function calculateRegionIndices(
  ...
  const filterCount = Object.values(regionMapping.currentFilters || {}).filter(
    a => a
  ).length;

  var regionColumnValues = filterCount
    ? regionColumn.values.map((a, i) => {
        for (const prop in regionMapping.currentFilters) {
          if (!defined(regionMapping.currentFilters[prop])) {
            continue;
          }

          const col = regionMapping._tableStructure.columns.find(
            a => a.id == prop
          );
          ...
          if (
            !filterValue(regionMapping.currentFilters[prop], col._values[i])
          ) {
            return 0;
          }
        }
  ...
  var timeColumn = regionMapping._catalogItem.terria.nowViewingTable
    ? null
  ...
  switch (regionDetail.regionProvider.serverType) {
    case "MVT":
      return function constructMVTFeatureInfo(feature) {
        ...
        var regionIndicesMapping = calculateRegionIndices(regionMapping);
        var rowNumber = regionIndicesMapping[uniqueId];
  ...
  switch (regionDetail.regionProvider.serverType) {
    case "MVT":
      ...
      regionImageryProvider = new MapboxVectorTileImageryProvider({
        ...
        srs: catalogItem.terria.activeProjection.srs,
    ...
    case "WMS":
      ...
      regionImageryProvider = new WebMapServiceImageryProvider({
        ...
        tilingScheme:
          catalogItem.terria.activeProjection.srs === "EPSG:4326"
