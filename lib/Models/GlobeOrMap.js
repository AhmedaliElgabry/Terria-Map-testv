...
const { default: OceansMaskHelper } = require("../Map/OceansMaskHelper");
...
var GlobeOrMap = function(terria) {
  ...
  this._maskLayerContext = {
    featureId: null,
    layerName: null,
    removeMaskCallback: null
  };
};
...
GlobeOrMap.prototype._removeMasksForLayers = function(layer) {
  const { layerName, removeMaskCallback } = this._maskLayerContext;
  ...
  if (defined(removeMaskCallback)) {
    ...
    this.terria?.onMaskChange?.(null);
  }
};

GlobeOrMap.prototype._createMaskAroundFeature = function(
  feature,
  context = {}
) {
  if (!defined(feature)) return;

  const featureId = feature.data?.id ?? feature.name ?? -1;
  const hash = hashEntity(feature, this.terria.clock);

  if (featureId == this._maskLayerContext.featureId) {
    /*
   If there is a mask already for the selected feature,
   then this code removes the mask
   this will toggle on and off the mask function
  */
    this._removeMasksForLayers();
    return;
  }

  // Before creating new mask, remove any existing ones
  this._removeMasksForLayers();

  const isSimpleGeometry = !!(feature.polygon || feature.polyline);
  ...
  if (imageryProvider instanceof MapboxVectorTileImageryProvider) {
    const maskContext = {
      ...
      hash,
      featureId,
      layerName: imageryProvider._layerName
    };

    if (defined(this.terria.cesium)) {
      createMaskForCesium.call(this, feature, oceansMaskEnabled, maskContext);
    } else if (defined(this.terria.leaflet)) {
      createMaskForLeaflet.call(this, feature, oceansMaskEnabled, maskContext);
    }
  } else if (
    !defined(this.supportsPolylinesOnTerrain) ||
    this.supportsPolylinesOnTerrain
  ) {
    // todo: support masking for this type as well
  }
};

function createMaskForLeaflet(feature, oceansMaskEnabled, maskContext) {
  const oceansMaskHelper = new OceansMaskHelper(this.terria);
  ...
  // Add mask for features
  var featureMask = new MapboxVectorCanvasTileLayer(
    feature.imageryLayer.imageryProvider.createMaskImageryProvider(
      this.terria,
      feature.data.id
    ),
    options
  ...
  var oceansMaskLayer = oceansMaskEnabled
    ? oceansMaskHelper.getLeafletMask()
    : null;

  featureMask.addTo(map);
  oceansMaskLayer && oceansMaskLayer.addTo(map);

  featureMask.bringToFront();

  this._maskLayerContext = {
    ...maskContext,
    removeMaskCallback: function() {
      map.removeLayer(featureMask);
      oceansMaskLayer && map.removeLayer(oceansMaskLayer);
    }
  };

  this.terria?.onMaskChange?.({
    value: maskContext.featureId,
    text: tryGetNameFromFeature(feature, maskContext)
  });
  return featureMask;
}

function createMaskForCesium(feature, oceansMaskEnabled, maskContext) {
  const oceansMaskHelper = new OceansMaskHelper(this.terria);

  var featureMask = new ImageryLayer(
    feature.imageryLayer.imageryProvider.createMaskImageryProvider(
      this.terria,
      feature.data.id
    ),
  ...
  var oceanMask = oceansMaskEnabled ? oceansMaskHelper.getCesiumMask() : null;

  var scene = this.terria.cesium.scene;
  scene.imageryLayers.add(featureMask);
  oceanMask && scene.imageryLayers.add(oceanMask);

  this._maskLayerContext = {
    ...maskContext,
    removeMaskCallback: function() {
      scene.imageryLayers.remove(featureMask);
      oceanMask && scene.imageryLayers.remove(oceanMask);
    }
  };
...
function tryGetNameFromFeature(feature, context) {
  ...
  if (feature._properties) {
    const key =
      Object.keys(feature._properties).find(a => a.includes("name")) ||
      Object.keys(feature._properties).find(a => a.includes("code")) ||
      Object.keys(feature._properties).find(a => a.includes("id")) ||
      Object.keys(feature._properties)?.[0];
    return feature._properties[key]._value;
  }
}

/**
 * If feature parameter is NOT provided, Returns a boolean indicating if a mask is on or off.
 * If the feature parameter is provided, Returns a boolean indicating if a mask is created
 * with a focus on specifically this feature, i.e, area around feature is masked but feature itself is in clear (focus) mode
 * @param {*} feature
 * @returns {Boolean}
 */
GlobeOrMap.prototype._isMaskOn = function(feature) {
  const { layerName, removeMaskCallback, featureId } = this._maskLayerContext;
  if (feature) {
    return featureId == feature.data?.id && removeMaskCallback;
  }

  return layerName && removeMaskCallback;
};
