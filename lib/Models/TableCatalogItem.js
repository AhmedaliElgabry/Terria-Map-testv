...
const isCommonMobilePlatform = require("../Core/isCommonMobilePlatform");
...
var TableCatalogItem = function(terria, url, options) {
  ...
  /**
   * Indicates if the table renderer should be used or not to visualize this tabular data.
   * @type {Boolean}
   * @default {false}
   */
  this.isTabular = defaultValue(options.isTabular, false);
  ...
  /**
   * To configure how this tablular catalog item is displayed in the table view.
   *
   */
  this.tabularViewSettings = defaultValue(options.tabularViewSettings, {});
  ...
  knockout.defineProperty(this, "canToggleTime", {
    get: function() {
      return (
        defined(this.customProperties.canToggleTime) &&
        this.customProperties.canToggleTime &&
        (defined(this._tableStructure) &&
          this._tableStructure.hasLatitudeAndLongitude)
      );
    }
  });
...
TableCatalogItem.defaultSerializers.tableStyle = function(
  ...
  if (
    (item.type === "csv" || item.type === "smart-csv") &&
...
TableCatalogItem.defaultSerializers.canToggleTime = function() {
  // Don't serialize.
};
...
TableCatalogItem.defaultPropertiesForSharing.push("isTabular");
...
TableCatalogItem.prototype.initializeFromTableStructure = function(
  ...
  if (!item.isMappable && item.isCsvForCharting) {
    makeChartable();
    return;
  }

  if (!item.isMappable && item.isTabular) {
    makeTabular.call(this);
    return;
  }
  ...
  // Remove existing datasource before creating a new one
  if (defined(item._regionMapping)) {
    item._hide();
    item._regionMapping = null;
  }
...
function createDataSourceForLatLong(item, tableStructure) {
  ...
  if (defined(item._dataSource)) {
    if (item.terria.dataSources.contains(item._dataSource)) {
      this._hide();
    }
  }
...
TableCatalogItem.prototype.setColorColumns = function() {
  if (!defined(this._tableStructure)) {
    return;
  }
  var tableStructure = this._tableStructure;
...
function ensureActiveColumnForNonSpatial(item) {
  ...
  suitableColumns.forEach(suitableColumn => {
    if (!suitableColumn.isActive) {
      suitableColumn.toggleActive();
    }
  });
  // if (
...
TableCatalogItem.prototype.getNextColor = function() {
  ...
  var tabularItems = this._terria.nowViewing.tabularItems;
  var itemsToColor = [...catalog.chartableItems, ...tabularItems];

  var colorsUsed = itemsToColor.map(function(item) {
...
/**
 * Enables column multi-select
 * Saves the active columns on map view to a temp storage
 * Restores tabular view active columns if they are present
 * @returns Promise
 */
TableCatalogItem.prototype.prepareTabularView = function() {
  return new Promise(resolve => {
    const columnSettings = this.tabularViewSettings.grid?.columns || {};
    ...
    let enabledCount = 0;
    this._tableStructure.columns
      // todo: Introduce a new var type which explicitly allows hiding columns in tables.
      // .filter(a => a.type != VarType.HIDDEN)
      .forEach(col => {
        enabledCount = setTabularColumnVisibility(
          columnSettings,
          col,
          enabledCount
        );
...
TableCatalogItem.prototype.revertToMapView = function() {
  return Promise.resolve(true);
};

module.exports = TableCatalogItem;

function setTabularColumnVisibility(columnSettings, col, enabledCount) {
  if (columnSettings?.hasOwnProperty(col.id)) {
    col.isVisibleOnTableView = !columnSettings[col.id].hidden;
  } else {
    col.isVisibleOnTableView = true;
  }

  if (
    col.isVisibleOnTableView &&
    ![
      VarType.ADDR,
      VarType.REGION,
      VarType.ALT,
    ...
    const [MinVisibleColumnsOnWeb, MaxVisibleColumnsOnWeb] = [4, 9];
    col.isActiveOnTableView = isCommonMobilePlatform()
      ? enabledCount < MinVisibleColumnsOnWeb
      : enabledCount < MaxVisibleColumnsOnWeb;

    if (col.isActiveOnTableView) {
      enabledCount += 1;
    }
  }
