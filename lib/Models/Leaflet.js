...
Leaflet.prototype._initProgressEvent = function() {
  var onTileLoadChange = function() {
    ...
    this.map.eachLayer(function(layer) {
      if (layer._tiles) {
        const total = Object.keys(layer._tiles).length;

        const loaded = Object.keys(layer._tiles).filter(
          key => layer._tiles[key].loaded
        ).length;

        const remaining = total - loaded;

        if (layer._tileLoadTracker) {
          layer._tileLoadTracker.onChange(loaded, total);
        }
        // Count all tiles not marked as loaded
        tilesLoadingCount += remaining;
      }
    });
...
Leaflet.prototype.removeImageryLayer = function(options) {
  ...
  if (options.layer?.imageryProvider?._layerName)
    this._removeMasksForLayers(options.layer.imageryProvider._layerName);
};
