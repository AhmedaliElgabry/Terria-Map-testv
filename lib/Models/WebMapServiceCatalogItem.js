...
var WebMapServiceCatalogItem = function(terria) {
  ...
  /**
   * Gets or sets wether analysis should be turned on for this WMS item. This
   * proerty is part of FAO terriajs fork.
   * @type {Boolean}
   */
   ...
   * @default false
   */
  this.supportMultipleDimensionValues = false;

  /**
   * When supporting multiple dimension values, the possible combination
   * of values can be large. By sorting, we reduce the possibilities
   * which will make caching easier.
   */
  this.sortMultipleDimensionValues = true;

  /**
   * Property used to define CQL filter criteria.
   * It will be sent with each request to the wms endpoint
   */
  this.cql_filter = undefined;
...
WebMapServiceCatalogItem.prototype._getValuesThatInfluenceLoad = function() {
...
function loadFromCapabilities(wmsItem) {
  ...
  if (defined(crs)) {
    ...
    if (crsIsMatch(crs, wmsItem.terria.activeProjection.srs)) {
      srs = wmsItem.terria.activeProjection.srs;

      if (wmsItem.terria.activeProjection.srs == "EPSG:3857") {
        tilingScheme = new WebMercatorTilingScheme();
      } else if (wmsItem.terria.activeProjection.srs == "EPSG:4326") {
        tilingScheme = new GeographicTilingScheme();
      }
    }
  }
...
WebMapServiceCatalogItem.prototype.getNonTimeDimensions = function() {
  if (!defined(this.availableDimensions)) {
    return undefined;
  }
  const dimensions = {};
  const layerDimensions = this.availableDimensions[this.layers].filter(
...
WebMapServiceCatalogItem.prototype._load = function() {
  ...
  return promise.then(function(token) {
    ...
    if (!defined(that._rawMetadata) && defined(that.getCapabilitiesUrl)) {
      ...
      that._overwriteFromGetCapabilities = true;
      loadFromCapabilities(that);
    }
...
WebMapServiceCatalogItem.prototype._createImageryProvider = function(time) {
  ...
  if (defined(this.cql_filter)) {
    parameters = combine({ cql_filter: this.cql_filter }, parameters);
  }
