...
var CatalogItem = function(terria) {
  ...
  /**
   * Gets or sets a value indicating whether this data item is currently refreshing. An enabled data item needs refreshing.
   * @type {Boolean}
   */
  ...
  /**
   * Gets or sets a value indicating whether this data source can be refreshed
   * This property is observable.
   * @type {Boolean}
   */
  this.isRefresheable = true;
  ...
  /**
   * Gets or sets a flag indicating whether clock should be disabled and time should be flattened
   * @type {Boolean}
   */
  ...
  knockout.getObservable(this, "isRefreshing").subscribe(function(newValue) {
    isRefreshingChanged(this);
  }, this);
...
CatalogItem.defaultMetadata.isRefreshing = false;
...
CatalogItem.defaultPropertiesForSharing.push("isTimeDisabled");
...
CatalogItem.prototype.reproject = function() {
  console.log("CatalogItem.prototype.reproject Item", this);
  // Send the item to a service and return the reprojected item.
  // Serverside data reprojecction GDAL, Ogr2Ogr, PyProj,
  // Browser side reprojection Not Possible

  // Concerns: slow speed and large load time
  // Solution: The services that we use CKAN/GEOSERVER also suppport other projections I just change the URL and receive tiles immediatly
  //           GEOJSON's can be reprojected in the browser.
  // Reprojections must be either cached or be done on the service which is closest to files.
};
...
CatalogItem.prototype.toggleTime = function() {
  if (this._lastisTimeDisabled !== this.isTimeDisabled) {
    if (this.isTimeDisabled) {
      this.useOwnClock = true;
    }
    this._reload();
    this.isLoading = false;
    this.isShown = true;
    this._lastisTimeDisabled = this.isTimeDisabled;
  }
};
...
function isRefreshingChanged(catalogItem) {
  if (catalogItem.isRefresheable && catalogItem.isRefreshing) {
    var that = defined(catalogItem.nowViewingCatalogItem)
      ? catalogItem.nowViewingCatalogItem
      : catalogItem;
    var loadPromise = when
      .all([that.load(), catalogItem.waitForDisclaimerIfNeeded()])
      .then(function() {
        if (defined(that.nowViewingCatalogItem)) {
          that.terria.nowViewing.items.remove(that);
        }
        if (typeof that.refresh === "function") {
          that.refresh();
        } else {
          // if refresh is not available recreate layer
          that._imageryLayer = undefined;
          if (defined(that.regionMapping)) {
            that.regionMapping._imageryLayer = undefined;
          }
          that._enable();
        }

        that.terria.currentViewer.addAttribution(that.attribution);
        if (defined(that.imageryLayer)) {
          that.imageryLayer.featureInfoTemplate = that.featureInfoTemplate;
        }
        raiseErrorOnRejectedPromise(
          that.terria,
          when(that._loadForEnablePromise, function() {
    ...
    raiseErrorOnRejectedPromise(that.terria, loadPromise);
    loadPromise.always(function() {
      that.terria.currentViewer.notifyRepaintRequired();
      that.isRefreshing = false;
      resolvedOrRejected = true;
      that._loadForEnablePromise = undefined;
    });
  } else {
    //if the item is not refreshable create a temoprary disable schedule here
  }
}
      ...
      var loadPromise = when
        ...
        .then(function() {
          if (catalogItem.isEnabled) {
            ...
            if (
              defined(catalogItem.isTimeDisabled) &&
              catalogItem.isTimeDisabled
            ) {
