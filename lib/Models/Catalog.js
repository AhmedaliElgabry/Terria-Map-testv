  ...
  return shareKeys.reduce(
    function(aggregatedPromise, shareKey) {
      return aggregatedPromise.then(
        function() {
          const index =
            this.shareKeyIndex[shareKey] || this.shareKeyIndex[shareKey.trim()];

          if (this.shareKeyIndex[shareKey]) {
            return this.shareKeyIndex[shareKey].load();
          } else {
            this.terria.hasUnauthorizedAccessForSharedData = true;
          }
        }.bind(this)
      );
    }.bind(this),
...
Catalog.prototype._updateAndEnable = function(shareKey, itemJson) {
  var existingMember =
    this.shareKeyIndex[shareKey] || this.shareKeyIndex[shareKey.trim()]; // Adding trim for backward compatability
  if (existingMember) {
    return existingMember
      .updateFromJson(itemJson)
      .then(() => {
        if (existingMember.refresh) {
          existingMember.refresh();
        }

        if (existingMember.onSmartCsvDimensionChange) {
          existingMember.onSmartCsvDimensionChange.valueHasMutated();
        } else if (
          existingMember.type === "remote-config-item" &&
          existingMember.hasOwnProperty("nowViewingCatalogItem") &&
          defined(existingMember.nowViewingCatalogItem) &&
          existingMember.nowViewingCatalogItem.type === "smart-csv" &&
          existingMember.nowViewingCatalogItem.onSmartCsvDimensionChange
        ) {
          existingMember.nowViewingCatalogItem.onSmartCsvDimensionChange.valueHasMutated();
        }
        /*
          // the fix on updateFromJSON preventing isEnabled event launching arbitrarily should 
          // make the code below unnecessary
          else if (
            existingMember.type === "remote-config-item" &&
            existingMember.hasOwnProperty("nowViewingCatalogItem") &&
            defined(existingMember.nowViewingCatalogItem)
          ) {
            existingMember.isEnabled &&
              existingMember.hardRefresh &&
              existingMember.hardRefresh();
          } else if (
            existingMember.type === "ckan-resource" &&
            existingMember.hasOwnProperty("nowViewingCatalogItem") &&
            defined(existingMember.nowViewingCatalogItem)
          ) {
            existingMember.isEnabled &&
        ...
        */
        return existingMember.load.bind(existingMember)();
      })
