...
const WMTS_WRAPPER_ENDPOINT = "https://data.apps.fao.org/map/wmts/wmts";
const WMTS_RETRY_LIMIT = 2;
...
var CesiumTileLayer = L.TileLayer.extend({
  initialize: function(imageryProvider, options) {
    ...
    if (
      this.imageryProvider instanceof WebMapTileServiceImageryProvider &&
      (this.imageryProvider._resource?.url || "").includes(
        WMTS_WRAPPER_ENDPOINT
      )
    ) {
  ...
  tryRefreshEmptyWmtsTiles: function(url, tile) {
    const transparencyCheckCanvas = document.createElement("canvas");

    const ctx = transparencyCheckCanvas.getContext("2d");

    transparencyCheckCanvas.width = tile.width;
    transparencyCheckCanvas.height = tile.height;
    ...
    const imageData = ctx.getImageData(
      0,
      0,
      transparencyCheckCanvas.width,
      transparencyCheckCanvas.height
    );

    const data = imageData.data;

    const isImageTransparent = this.isImageTransparent(data);

    const currentRetryCount = url.searchParams.has(
      "wmts_cache_busting_retry_count"
    )
      ? Number(url.searchParams.get("wmts_cache_busting_retry_count"))
      : 0;

    const newCount = currentRetryCount + 1;

    if (isImageTransparent && newCount <= WMTS_RETRY_LIMIT) {
      setTimeout(() => {
        console.info(
          `%cFound a transparent tile. Applying cache busting an empty tile. Retry number: ${newCount}`,
          "background: #fff; color:indianred; border:solid 1px indianred; border-radius: 4px; padding:5px 10px;font-family:monospace;",
          tile
        );

        url.searchParams.set("wmts_cache_busting_retry_count", newCount);
        url.searchParams.set("wmts_cache_busting_tx", new Date().getTime());
  ...
  _tileOnLoad: function(done, tile, e) {
    ...
    if (tile.complete && url.href.includes(WMTS_WRAPPER_ENDPOINT)) {
      try {
        this.tryRefreshEmptyWmtsTiles(url, tile);
      } catch (error) {
        console.error(error);
      }
    }
  ...
  createTile: function(coords, done) {
    ...
    /**
     * Commented to prevent missing tile image
