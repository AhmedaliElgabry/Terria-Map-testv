 ...
 */
var LeafletGeomVisualizer = function(
  leafletScene,
  entityCollection,
  filterContext
) {
...
LeafletGeomVisualizer.prototype.update = function(time) {
  ...
  for (var i = 0, len = entities.length; i < len; i++) {
    ...
    const disableTime = !!this.filterContext?.flattenTime;
...
LeafletGeomVisualizer.prototype._updatePoint = function(
  ...
  // In order for a point to be visible on the map,
  // either the entity must be available at that time or time must be disabled
  var show =
    disableTime ||
...
LeafletGeomVisualizer.prototype._updatePolygon = function(
  ...
  var layer = details.layer;
  var coordinatesWithHoles = details.coordinatesWithHoles || [];
  ...
  if (!defined(layer)) {
    ...
    let cover = hierarchyToLatLngs(hierarchy);

    if (hierarchy.holes && hierarchy.holes.length) {
      var holes = hierarchy.holes.map(hierarchyToLatLngs); //.map(a => [a.lat, a.lng]);

      details.coordinatesWithHoles = coordinatesWithHoles = [
        cover.map(a => [a.lat, a.lng]),
        ...holes.map(a => a.map(b => [b.lat, b.lng]))
      ];

      layer = details.layer = L.polygon(coordinatesWithHoles, polygonOptions);
    } else {
      layer = details.layer = L.polygon(cover, polygonOptions);
    }
  ...
  if (hierarchy !== details.lastHierachy) {
    if (coordinatesWithHoles && coordinatesWithHoles.length) {
      layer.setLatLngs(coordinatesWithHoles);
    } else {
...
function convertEntityPositionsToLatLons(positions) {
  let result = convertEntityPositionsToLatLonsOriginal(positions);

  // If this result contains lat/lng close to the poles, then use
  // the modified (simpler) algorithm instead
  if (
    result.map(a => Math.abs(a.lat)).some(b => b >= 88) ||
    result.map(a => Math.abs(a.lng)).some(b => b > 179)
  ) {
    result = convertEntityPositionsToLatLonsModified(positions);
  }

  return result;
}

function convertEntityPositionsToLatLonsModified(positions) {
  var latlngs = [];
  const R = 6371 * 1000; // approximate radius of the earth

  for (const position of positions) {
    const { x, y, z } = position;
    const latRadians = Math.asin(z / R);
    const lngRadians = Math.atan2(y, x);

    latlngs.push(
      L.latLng(
        CesiumMath.toDegrees(latRadians),
        CesiumMath.toDegrees(lngRadians)
      )
    );
  }
...
/* This function converts cartesian x,y,z to lat, lng
   However, it fails around extreme latitudes and longitudes.
   Disabled for now. Replaced by convertEntityPositionsToLatLonsModified
   which works as expected across those lat,lng values.
*/
function convertEntityPositionsToLatLonsOriginal(positions) {
...
LeafletVisualizer.prototype.visualizersCallback = function(
  ...
  var filterContext = dataSource.filterContext;
  return [new LeafletGeomVisualizer(leafletScene, entities, filterContext)];
};
