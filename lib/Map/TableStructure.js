...
var strftime = require("../ThirdParty/strftime");
...
var TableStructure = function(name, options) {
  ...
  this.regex = options.regexFormatter;
...
Object.defineProperties(TableStructure.prototype, {
  ...
  activeTimeColumnNameIdOrIndex: {
    ...
    set: function(nameIdOrIndex) {
      ...
      if (defined(timeColumnToActivate)) {
        const values = timeColumnToActivate.values;
        const possibleDateValues = new Set(values);

        if (possibleDateValues.size <= 1) {
          // this means we do not have unique date values in the data to construct a timeline (we need at least two)
          console.warn(
            "Insufficient data to create a timeline. At least two unique dates are required to construct a timeline."
          );
          this._activeTimeColumnNameIdOrIndex = undefined;
          return;
        }
...
/**
 * Returns a news Csv String without the column
 *
 * @param {String} columnName Column to remove.
 */
TableStructure.prototype.removeColumn = function(csvStr, columnName) {
  const csvArray = csv.toArrays(csvStr);
  const masterCsvHeader = csvArray[0];
  const index = masterCsvHeader.indexOf(columnName);
  csvArray.forEach(a => a.splice(index, 1));
  return arrayToCSV(csvArray);
};

/**
 * merges two csv string. First pick from master csv and then fill from inbound csv to get the latest value for every column.
 *
 * Next challeneg is to adjust the xAxis. If the area has changed the xAxis must also adjust.
 */
TableStructure.prototype.mergeCsvStrings = function(master, inbound, options) {
  const masterCsv = csv.toArrays(master);
  const inboundCsv = csv.toArrays(inbound);
  const masterCsvHeader = masterCsv.shift();
  const inboundCsvHeader = inboundCsv.shift();
  const xAxisIndex = masterCsvHeader.indexOf(options.xAxis) || 0; // pull the xAxis requiredx info from the tablestructure here.
  const mergeCsvHeader = Array.from(
    new Set(masterCsvHeader.concat(inboundCsvHeader).flat())
  );

  const mergedxAxis = Array.from(
    new Set(
      masterCsv
        .map(x => x[xAxisIndex])
        .concat(inboundCsv.map(x => x[xAxisIndex]))
        .flat()
    )
  ).sort((a, b) => new Date(a) - new Date(b));

  const gapFilledXAxis = fillupXAxis(mergedxAxis, this.regex);
  const headers = {
    masterCsvHeader: masterCsvHeader,
    inboundCsvHeader: inboundCsvHeader,
    mergeCsvHeader: mergeCsvHeader
  };
  const finalArray = generateNewTableFromCsvStrings(
    headers,
    gapFilledXAxis,
    masterCsv,
    inboundCsv,
  ...
  finalArray.unshift(mergeCsvHeader);
  return arrayToCSV(finalArray);
};

function generateNewTableFromCsvStrings(
  headers,
  xAxes,
  master,
  inbound,
  ...
  const { masterCsvHeader, inboundCsvHeader, mergeCsvHeader } = headers;
  return Array(xAxes.length)
    .fill(null)
    .map((item, i) => {
      return Array(mergeCsvHeader.length)
        .fill(null)
        .map((columnval, columnIndex) => {
          if (columnIndex === xAxisIndex) {
            return xAxes[i];
          } else {
            if (
              masterCsvHeader.includes(mergeCsvHeader[columnIndex]) &&
              master.map(x => x[xAxisIndex]).includes(xAxes[i]) &&
              !inboundCsvHeader.includes(mergeCsvHeader[columnIndex])
            ) {
              //master pass
              const valueIndex = masterCsvHeader.indexOf(
                mergeCsvHeader[columnIndex]
              );
              const rowIndex = master.map(x => x[xAxisIndex]).indexOf(xAxes[i]);
              columnval = master[rowIndex][valueIndex];
            }
            if (
              inboundCsvHeader.includes(mergeCsvHeader[columnIndex]) &&
              inbound.map(x => x[xAxisIndex]).includes(xAxes[i])
            ) {
              //inbound pass
              const valueIndex = inboundCsvHeader.indexOf(
                mergeCsvHeader[columnIndex]
              );
              const rowIndex = inbound
                .map(x => x[xAxisIndex])
                .indexOf(xAxes[i]);
              columnval = inbound[rowIndex][valueIndex];
            }
            return columnval;
          }
        });
...
//need to rovide a better logic using APIs, infor karl about inability to fill up x Axis without regex
function fillupXAxis(mergedxAxis, regex) {
  if (
    mergedxAxis.length > 2 &&
    moment(mergedxAxis[0]).isValid() &&
    moment(mergedxAxis[1]).isValid() &&
    isNaN(mergedxAxis[0]) &&
    isNaN(mergedxAxis[1]) &&
    regex
  ) {
    const preciseDiff = moment.preciseDiff(
      mergedxAxis[1],
      mergedxAxis[0],
      true
    );
    const startDate = moment(mergedxAxis[0]);
    const endDate = moment(mergedxAxis[mergedxAxis.length - 1]);
    const finalArray = [];
    var currentDate = startDate;

    while (currentDate.isSameOrBefore(endDate, "day")) {
      // currentDate = startDate;
      finalArray.push(strftime(regex, currentDate.toDate()));
      const { days, firstDateWasLater, ...rest } = preciseDiff;
      const allZeros = Object.values(rest).every(e => e === 0);
      if (allZeros && days === 10) {
        // you only wanna add 10 days
        if (Number(currentDate.format("D")) === 21) {
          currentDate = currentDate.add("months", 1).startOf("month");
        } else {
          currentDate.add("days", 10);
        }
      } else {
        // eslint-disable-next-line no-loop-func
        Object.keys(preciseDiff).forEach(key => {
          currentDate.add(preciseDiff[key], key);
        });
      }
    }
    return finalArray;
...
function arrayToCSV(arr, delimiter = ",") {
  return arr
    .map(v =>
      v.map(x => (defined(x) && x !== "" ? `"${x}"` : null)).join(delimiter)
    )
    .join("\n");
}
...
function getColumnOptions(name, tableStructure, columnNumber) {
  ...
  var description = defaultValue(columnOptions.description, "");
