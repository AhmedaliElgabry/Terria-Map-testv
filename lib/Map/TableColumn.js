...
var typeHintSet = [
  ...
  { hint: /^month|xaxis|postcode|poa|(.*_code)$/i, type: VarType.ENUM }
];
...
var TableColumn = function(name, values, options) {
  ...
  // // Type hint set has issues. It assigns any column name that contains keyword date as time type. This is wrong talk to this with karl
  // if (this._type === VarType.TIME && values.some(isNumerical)) {
  ...
  // disabled this so that the grouping may work correctly
