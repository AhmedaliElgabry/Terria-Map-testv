...
import { createWindowsManagementState } from "../Models/WindowManagement";
...
export default class ViewState {
  constructor(options) {
    ...
    /**
     * Gets or sets the window/dialogs management state
     */
    this.modalWindows = createWindowsManagementState();
    ...
    knockout.defineProperty(this, "isChartPanelVisible", {
      get: function() {
        return this.terria.viewType == VisualizationType.CHART;
      },
      set: function(value) {
        this.terria.viewType = value
    ...
    knockout.defineProperty(this, "chartIsOpen", {
      get: function() {
        ...
        // return chartableItems.length > 0;
        const chartableColumnExists = chartableItems.some(
          item => item.isCsvForCharting
        );
    ...
    this._catalogItemAddedSubscription = knockout
      .getObservable(terria, "catalogItemAdded")
      .subscribe(item => {}, this);

    this._catalogItemRemovedSubscription = knockout
      .getObservable(terria, "catalogItemRemoved")
      .subscribe(item => {
        if (
          this.terria.isCommonMobilePlatform &&
          this.terria.nowViewing.items.filter(item => item.isCsvForCharting)
            .length == 0
        ) {
          this.isChartPanelVisible = false;
        }
      }, this);
    ...
    ActionManager.on(TerriaAction.OpenExplorePanel, () => this.openAddData())
      .on(TerriaAction.CloseExplorePanel, () => this.closeCatalog())
      .on(TerriaAction.OpenUserData, () => this.openUserData());
  }
  ...
  dispose() {
    ...
    this._catalogItemAddedSubscription.dispose();
    this._catalogItemRemovedSubscription.dispose();
    ...
    this._chartPanelVisibleHandler.dispose();
